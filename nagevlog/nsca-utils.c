...
int encrypt_init(char *password,int encryption_method,char *received_iv,struct crypt_instance **CIptr){
		...
		if( *CIptr ) return OK; /* already done it */
        ...
        *CIptr=CI;
		...
		bzero(CI,sizeof(struct crypt_instance));
...
#ifdef NSCADEBUG
        ntsl_log_info("Attempting to initialize '%s' crypto algorithm...",CI->mcrypt_algorithm);
#endif
...
#ifdef NSCADEBUG
        ntsl_log_info("Using '%s' as crypto algorithm...",CI->mcrypt_algorithm);
#endif
