...
*/
int send_nsca(char * server_name, int server_port,  char * svc_description, int return_code, char * plugin_output ) {
	...
	if( !*svc_description || (return_code>3) ) { return(0); }
	...
	/* we couldn't connect */
	if(result!=STATE_OK){
		ntsl_log_error("Error: Could not connect to host %s on port %d\n",server_name,server_port);
		return 2;
	        }
	/* read the initialization packet containing the IV and timestamp */
	result=read_init_packet(sd);
	if(result!=OK){
       ntsl_log_error("Error: Could not read init packet from server");
		closesocket(sd);
		return 2;
	        }
	...
	/* initialize encryption/decryption routines with the IV we received from the server */
		...
		ntsl_log_error("Error: Failed to initialize encryption libraries for method %d",encryption_method);
		closesocket(sd);
		return 1;
	        }

#ifdef NSCADEBUG
	ntsl_log_info("Initialized encryption routines [%s][%d]\n",password,encryption_method);
#endif
	...
	/**** WE'RE CONNECTED AND READY TO SEND ****/
		...
		strcpy_s(&send_packet.host_name[0],MAX_HOSTNAME_LENGTH,nscahostname);
		strcpy_s(&send_packet.svc_description[0],MAX_DESCRIPTION_LENGTH,svc_description);
		strcpy_s(&send_packet.plugin_output[0],MAX_PLUGINOUTPUT_LENGTH,plugin_output);

		/* use timestamp provided by the server */
		send_packet.timestamp=(u_int32_t)htonl((u_long)packet_timestamp);
	...
	/* exit cleanly */
	do_cleanup();  /* we need to clean up since we'll use it again later with a different IV */
	return 0;
}
...
int read_init_packet(SOCKET sock){
        int rc;
        ...
        if(rc<=0){
                ntsl_log_error("Error (%d): Server closed connection before init packet was received\n",-rc);
                return NSCA_ERROR;
                }
...
void alarm_handler(int sig){
	ntsl_log_error("Error: Timeout after %d seconds\n",socket_timeout);
}
...
int read_config(){
	...
	size = sizeof(DWORD);
	rc  = RegQueryValueEx(hReg, "logAlerts", NULL, NULL, (unsigned char*)&logalerts, &size);

	/* encryption method */
		size = sizeof(DWORD);
	rc  = RegQueryValueEx(hReg, "encryptionMethod", NULL, NULL, (unsigned char*)&val, &size);
   	if ( rc == ERROR_SUCCESS  ) {
	...
	rc  = RegQueryValueEx(hReg, "nscaPassword", NULL, NULL, password, &size);

	/* my hostname for NSCA */
	size = sizeof(nscahostname);
	rc  = RegQueryValueEx(hReg, "MyHostname", NULL, NULL, nscahostname, &size);
