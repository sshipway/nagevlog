...
int my_connect(char *host_name,int port,int *sd,char *proto){
	...
	if(!my_inet_aton(host_name,&servaddr.sin_addr)){
		...
		if(hp==NULL){
//			AfxMessageBox(_T("Invalid host name"),MB_OK|MB_ICONERROR);
...
//		AfxMessageBox(_T("Cannot map  protocol number"),MB_OK|MB_ICONERROR);
		return STATE_UNKNOWN;
	        }
...
//			break;
//		default:
//			AfxMessageBox(_T("Connection refused or timed out\n"),MB_OK|MB_ICONERROR);
//		        }
...
int recvall(int s, char *buf, int *len, int timeout){
	...
	while(total<*len){
		...
		if(n==-1) {
			e = WSAGetLastError();
			if ((e==EAGAIN) || (e==997) || (e==WSAEWOULDBLOCK)  
#ifdef WINDOWS_NT
				) {
#else
